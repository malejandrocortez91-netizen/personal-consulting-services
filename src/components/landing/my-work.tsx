             ...
             return (
                <Card key={item.id} className="relative animate-unfold border shadow-lg" style={{ animationDelay: `${index * 150}ms`}}>
                  ...
                  <CardContent className="grid grid-cols-1 items-start gap-8 lg:grid-cols-2 lg:gap-12 pt-2">
                      <div className={cn("space-y-6", item.id === 'appointment-dashboard' ? 'lg:order-last' : (isReversed ? 'lg:order-last' : ''))}>
                      ...
                      {images.length > 0 && (
                        <div className={cn("relative h-96 w-full", item.id === 'appointment-dashboard' ? 'lg:order-first' : (isReversed ? 'lg:order-first' : ''))}>
